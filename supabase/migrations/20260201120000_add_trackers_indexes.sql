...
ON public.trackers(user_id, is_active) 
...
ON public.trackers(display_order) 
WHERE deleted_at IS NULL;
...
ON public.trackers(updated_at DESC) 
WHERE deleted_at IS NULL;
...
ON public.tracker_shares(shared_with_user_id);
...
ON public.tracker_shares(tracker_id);
...
ON public.tracker_shares(tracker_id, shared_with_user_id);
...
ON public.entries(tracker_id, recorded_at DESC) 
WHERE deleted_at IS NULL;
...
ON public.entries(recorded_at DESC) 
WHERE deleted_at IS NULL;
...
ON public.entries(tracker_id, recorded_at) 
WHERE deleted_at IS NULL;
...
COMMENT ON INDEX public.idx_trackers_user_id_active IS 'Optimizes tracker listing with is_active filter';
COMMENT ON INDEX public.idx_trackers_data_type IS 'Optimizes tracker filtering by data_type';
COMMENT ON INDEX public.idx_trackers_display_order IS 'Optimizes tracker sorting by display_order';
COMMENT ON INDEX public.idx_trackers_name_lower IS 'Optimizes case-insensitive name sorting';
COMMENT ON INDEX public.idx_trackers_created_at IS 'Optimizes tracker sorting by creation date';
COMMENT ON INDEX public.idx_trackers_updated_at IS 'Optimizes tracker sorting by update date';
COMMENT ON INDEX public.idx_tracker_shares_user IS 'Optimizes shared tracker lookups by user';
COMMENT ON INDEX public.idx_tracker_shares_tracker IS 'Optimizes share lookups by tracker';
COMMENT ON INDEX public.idx_tracker_shares_unique IS 'Prevents duplicate tracker shares';
COMMENT ON INDEX public.idx_entries_tracker_recorded IS 'Optimizes entry queries by tracker and date';
COMMENT ON INDEX public.idx_entries_recorded_at IS 'Optimizes entry date range queries';
COMMENT ON INDEX public.idx_entries_tracker_stats IS 'Optimizes tracker statistics calculations';
